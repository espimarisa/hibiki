...
export class UnverifyCommand extends Command {
  ...
  async run(msg: Message<TextChannel>, pargs: ParsedArgs[], args: string[]) {
    ...
    if (!member.roles?.includes(guildconfig.verifiedRole)) {
      return msg.createEmbed(
        msg.locale("global.ERROR"),
        msg.locale("moderation.UNVERIFY_DOESNTHAVE", { member: msg.tagUser(member.user) }),
        "error",
      );
    }
    ...
    try {
      ...
      return msg.createEmbed(msg.locale("global.ERROR"), msg.locale("moderation.UNVERIFY_FAILED"), "error");
    }
    ...
    msg.createEmbed(
      ...
      msg.locale("moderation.UNVERIFY_SUCCESS", { member: msg.tagUser(member.user) }),
      "success",
    );
  }
}
