...
export class NHentaiCommand extends Command {
  ...
  async run(msg: Message<TextChannel>, pargs: ParsedArgs[], args: string[]) {
    ...
    if (args.length && /\d{1,6}/.test(args.join(" "))) {
      ...
      if (!body || !body?.data) return msg.createEmbed(msg.locale("global.ERROR"), msg.locale("nsfw.NHENTAI_NOTFOUND"), "error");
      book = body.data;
    } else {
      // Searches for a doujin
      const id = await axios.get(`https://nhentai.net/api/galleries/search?query=${query}`).catch(() => {});
      if (!id || !id.data?.result?.length) return msg.createEmbed(msg.locale("global.ERROR"), msg.locale("nsfw.NHENTAI_NOTFOUND"), "error");
    ...
    // Total pages
    fields.push({
      name: msg.locale("nsfw.NHENTAI_TOTALPAGES"),
    ...
    if (book.num_favorites?.length) {
      fields.push({
        name: msg.locale("nsfw.NHENTAI_FAVORITES"),
    ...
    if (book.upload_date) {
      fields.push({
        name: msg.locale("global.UPLOADED"),
        value: dateFormat(book.upload_date * 1000, msg.locale),
        inline: false,
      });
    }
    // Tags
    if (book.tags?.length) {
      fields.push({
        name: msg.locale("nsfw.NHENTAI_TAGS"),
        value: `${book.tags
          .slice(0, 10)
          .map((t: Record<string, string>) => `\`${t.name}\``)
          .join(",")}${book.tags.length > 10 ? msg.locale("nsfw.NHENTAI_OVERFLOW", { length: book.tags.length - 10 }) : ""}`,
        inline: false,
      });
    }
    ...
    const omsg = await msg.channel.createMessage({
      embed: {
        ...
        fields: fields,
        author: {
          name: book.title.pretty || book.title.english || msg.locale("global.UNKNOWN"),
        ...
        footer: {
          text: msg.locale("global.RAN_BY", {
            ...
            extra: msg.locale("nsfw.NHENTAI_STARTREADING"),
          }),
    ...
    pagify(
      book.images.pages.map((_p: number, i: number) => ({
        color: msg.convertHex("general"),
        title: `📖 ${book.title.pretty || book.title.english || msg.locale("global.UNKNOWN")}`,
        image: {
