...
export class CryptoCommand extends Command {
  ...
  async run(msg: Message<TextChannel>, pargs: ParsedArgs[]) {
    ...
    if (!body || !body.data?.[currency?.id]) {
      return msg.createEmbed(msg.locale("global.ERROR"), msg.locale("utility.CRYPTO_ERROR"), "error");
    }
    ...
    if (to && data?.[to]) {
      ...
      return msg.createEmbed(
        `💰 ${msg.locale("utility.CRYPTO_TO", { base: currency.name, to: toCurrency.name })}`,
        msg.locale("utility.CRYPTO_EQUALSTO", { base: currency.name, value: data[to], to: toCurrency.name }),
      );
    }
    ...
    msg.channel.createMessage(
      {
        embed: {
          title: `💰 ${currency.name}`,
          description: msg.locale("utility.CRYPTO_UPDATEDAT", { time: dateFormat(data.last_updated_at * 1000, msg.locale) }),
          ...
          fields: [
            ...
            {
              name: msg.locale("utility.CRYPTO_24HRCHANGE"),
