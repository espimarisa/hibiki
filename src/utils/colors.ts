/**
 * @file Colors
 * @description Converts hex colours between binary, hex, hsv, etc. and returns their names
 * @module utils/colors
 */
...
export function xyzToLAB(x: number, y: number, z: number) {
  ...
  if (X2 > 0.008856) X2 = Math.pow(X2, 1 / 3);
  else X2 = 7.787 * X2 + 16 / 116;
  if (Y2 > 0.008856) Y2 = Math.pow(Y2, 1 / 3);
  else Y2 = 7.787 * Y2 + 16 / 116;
  if (Z2 > 0.008856) Z2 = Math.pow(Z2, 1 / 3);
...
export function colorName(colorHex: string) {
  colorHex = hexColorRegex.exec(colorHex)[1];
  const colorRGB = rgbToLAB(hexToRGB(colorHex));
  if (colorRGB.find((rgb) => isNaN(rgb))) return ["000000", "Invalid Color"];

  let closest;
  let closestDiff = 0xffffff;
  colors.forEach((clr) => {
    // Finds the difference
    const diff = Math.sqrt(
      Math.pow(parseInt(clr[1][0]) - colorRGB[0], 2) +
        Math.pow(parseInt(clr[1][1]) - colorRGB[1], 2) +
        Math.pow(parseInt(clr[1][2]) - colorRGB[2], 2),
    );
...
(colors as unknown) = colors.map((color) => [color[0], rgbToLAB(hexToRGB(color[0])), color[1]]);
