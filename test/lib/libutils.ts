import { DataTypeByIndex, DataTypes } from "./Network/DataTypes";
import { IPCPacket } from "./Network/Packets/IPCPacket";
import { InvalidPacket } from "./Network/Packets/InvalidPacket";
...
export function handleNetworkMessage(message: string) {
  let parsed;
  try {
    parsed = JSON.parse(message);
  } catch (e) {
    return new PlainTextPacket(message);
  }
