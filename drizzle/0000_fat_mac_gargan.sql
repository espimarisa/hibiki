CREATE TABLE "guild_config" (
	...
	CONSTRAINT "guild_config_guild_id_snowflake_check" CHECK ("guild_config"."guild_id" ~ '^(?<id>\d{17,20})$'),
	CONSTRAINT "guild_config_starboard_channel_snowflake_check" CHECK (("guild_config"."starboard_channel" IS NULL OR "guild_config"."starboard_channel" ~ '^(?<id>\d{17,20})$')),
...
CREATE TABLE "starboard_entries" (
	...
	CONSTRAINT "starboard_entries_guild_id_snowflake_check" CHECK ("starboard_entries"."guild_id" ~ '^(?<id>\d{17,20})$'),
	CONSTRAINT "starboard_entries_message_id_snowflake_check" CHECK ("starboard_entries"."message_id" ~ '^(?<id>\d{17,20})$'),
	CONSTRAINT "starboard_entries_starboard_message_id_snowflake_check" CHECK ("starboard_entries"."starboard_message_id" ~ '^(?<id>\d{17,20})$')
);
--> statement-breakpoint
CREATE TABLE "starboard_reactions" (
	...
	CONSTRAINT "starboard_reactions_message_id_snowflake_check" CHECK ("starboard_reactions"."message_id" ~ '^(?<id>\d{17,20})$'),
	CONSTRAINT "starboard_reactions_user_id_snowflake_check" CHECK ("starboard_reactions"."user_id" ~ '^(?<id>\d{17,20})$')
);
--> statement-breakpoint
CREATE TABLE "user_config" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
	"user_id" text NOT NULL,
	CONSTRAINT "user_config_user_id_snowflake_check" CHECK ("user_config"."user_id" ~ '^(?<id>\d{17,20})$')
);
--> statement-breakpoint
